...
- FastAPI / 开放 API 文档：<http://localhost:8080/api/docs>
...
管理员进入 `/admin` 的“开放 API”区域，创建 API 应用并选择关联账号、授权目录和接口权限。系统只在创建或轮换时完整展示一次 API Key，数据库不保存明文密钥。
...
```bash
curl http://localhost:8080/api/v1/open/nodes \
...
第一版开放浏览、文件信息、新建文件夹、上传、下载、重命名、移动和移入回收站。接口只允许访问授权目录及其后代；即使知道其他文件 UUID 也无法越权。字段、请求示例和错误响应以 `/api/docs` 中“开放 API - 文件管理”分组为准。
