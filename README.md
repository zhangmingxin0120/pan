...
- 开放 API 文档手册：<http://localhost:8080/api-docs>
...
```bash
curl http://localhost:8080/api/v1/open/findlist \
...
```bash
curl -X POST http://localhost:8080/api/v1/open/upload \
  -H "Authorization: Bearer $PAN_API_KEY" \
  -F "file=@./example.pdf"
```
...
使用 Certbot 配置 HTTPS 后，将 `.env` 中的 `COOKIE_SECURE` 改为 `true`、`CORS_ORIGINS` 改为实际 HTTPS 域名，并重新运行 `PAN_NO_OPEN=1 ./restart.sh`。生产环境不要在纯 HTTP 下登录。服务器防火墙或云安全组只开放 `80` 和 `443`；不要将 PostgreSQL、`8000` 或 `8080` 暴露到公网。
